...
encode = function(traj.xyz) {
	...
	sa_ali.v = apply(traj.xyz, 1, function(traj_conf.xyz) {
	  ## fit all conformation fragments to all alphabet fragments
 	  ...
 	  rmsd_m = sapply(1:nConfFrag, function(x) {
		  ...
		  sapply(1:nSAFrag, function(y) {
		  	#kabsch_R(traj_conf.xyz[((3 * x) - 2):((3 * x) + 9)], sadata_o@fragment_coordinates[[y]]);
		  	kabsch_C(4, traj_conf.xyz[((3 * x) - 2):((3 * x) + 9)], sadata_o@fragment_coordinates[[y]]);
	  	});
	  });
	  ...
	  ## fragment string
	  sa_chars.v = sadata_o@fragment_letters[rmsd_min.ix];
	  ## from vector of chars to (vector of) string
	  sa_string.v = paste(sa_chars.v, collapse = '')
